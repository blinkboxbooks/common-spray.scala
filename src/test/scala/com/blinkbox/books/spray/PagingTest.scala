...
class PagingTest extends FunSuite {
  ...
  test("links for first page, including this") {
    assert(links(numberOfResults = Some(20), offset = 0, count = 5, baseUrl, Map.empty, includeSelf = true).toSet
  ...
  test("links for first page, excluding this") {
    assert(links(numberOfResults = Some(20), offset = 0, count = 5, baseUrl, Map.empty, includeSelf = false).toSet
      === Set(PageLink("next", s"$baseUrl?count=5&offset=5")))
  }

  test("links for middle page, including this") {
    assert(links(numberOfResults = Some(20), offset = 5, count = 5, baseUrl, Map.empty, includeSelf = true).toSet
  ...
  test("links for middle page, excluding this") {
    assert(links(numberOfResults = Some(20), offset = 5, count = 5, baseUrl, Map.empty, includeSelf = false).toSet
      === Set(PageLink("prev", s"$baseUrl?count=5&offset=0"),
        PageLink("next", s"$baseUrl?count=5&offset=10")))
  }

  test("links for last page, including this") {
    assert(links(numberOfResults = Some(20), offset = 15, count = 5, baseUrl, Map.empty, includeSelf = true).toSet
  ...
  test("links for last page, excluding this") {
    assert(links(numberOfResults = Some(20), offset = 15, count = 5, baseUrl, Map.empty, includeSelf = false).toSet
  ...
  test("links when total number isn't known") {
    ...
    assert(links(numberOfResults = None, offset = 15, count = 5, baseUrl, Map.empty, includeSelf = true).toSet
      === Set(PageLink("this", s"$baseUrl?count=5&offset=15"),
        PageLink("prev", s"$baseUrl?count=5&offset=10"),
  ...
  test("links for first page, excluding this, including existing query parameters") {
    assert(links(numberOfResults = Some(20), offset = 0, count = 5, baseUrl, Map("test" -> "1"), includeSelf = false).toSet
